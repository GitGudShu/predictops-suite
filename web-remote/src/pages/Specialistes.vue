  ...
  <q-page>
    ...
    <div class="actions-bar" v-if="isAllowed && view !== 'visualize'">
      ...
      <div class="right-buttons">
        <Button v-if="view !== 'visualize'" @click="fetchActiveSpecialist" left-icon="mdi-refresh"
          bg-color="var(--sad-nightblue)" txt-color="white" />
      </div>
    </div>
    <VueDraggable v-model="cards" @end="onDragEnd" class="container" handle=".handle">
      <div class="q-mt-xl empty" v-if="cards.length === 0 && !creatingCard && view !== 'visualize'"
        ...
        <div class="text-bold text-h5 text-center text-black">Aucun.e.s spécialiste actif.ve</div>
        <div class="text-h6 text-center text-black" v-if="isAllowed">Ajoutez une carte de specialiste pour commencer
        </div>
        <Button btn-text="Ajouter une carte" @click="creatingCard = true" btn-size="md-btn" left-icon="fa-solid fa-plus"
      ...
      <div v-for="(card, index) in cards" :key="card.id" class="specialist-card">
        <div class="specialist-card-header drag-handle"
          ...
          <input v-if="card.editing" v-model="card.title" class="specialist-title-input" type="text"
        ...
        <div class="specialist-card-body">
          ...
          <input v-model="card.body" class="specialist-body-input" type="text" placeholder="Agent"
        ...
        <div class="specialist-card-footer editing" v-if="card.editing && isAllowed">
      ...
      <!-- New Card Form -->
      <div v-if="creatingCard" class="specialist-card">
        <div class="specialist-card-header"
          :style="{ 'background-color': 'transparent', 'color': 'var(--sad-nightblue)' }">
          ...
          <input v-model="newCard.title" class="specialist-title-input" type="text" placeholder="Titre"
            :style="{ 'width': 'calc(100% - 75px)' }" />
          <input type="color" v-model="newCard.color" class="color-picker" />
        </div>

        <div class="specialist-card-body">
          <input v-model="newCard.body" class="specialist-body-input" type="text" placeholder="Agent"
            v-if="!newCard.auto" />
          <input v-model="computedBody" class="specialist-body-input" type="text" placeholder="Agent"
            v-if="newCard.auto" readonly />
        ...
        <div class="specialist-card-footer editing" v-if="isAllowed">
...
const activeSpecialistData = ref([]);
...
const fetchActiveSpecialist = async () => {
  loading.value = true;
  try {
    ...
    // Fetch active specialist data excluding images
    const activeSpecialistResponse = await api.get(`/data/active-specialist?dpt=${dpt.value}`);
    activeSpecialistData.value = activeSpecialistResponse.data;
    ...
    cards.value = activeSpecialistData.value.map(item => ({
      id: item.id,
      title: item.title,
      body: item.body,
...
const loadCardImageWithCache = async (card, index) => {
  ...
  try {
    const imageResponse = await api.get(`/data/image/active_specialistes/${card.id}`);
...
const onDragEnd = async () => {
  ...
  try {
    ...
    const response = await api.patch(`/admin-cta/update-card-order`, { cards: cards.value, page: 'specialistes' });
...
const duplicateCard = async (card) => {
  try {
    ...
    const response = await api.post(`/admin-cta/create-active-specialist`, duplicatedCard);
    if (response.data.success) {
...
const saveCard = async (card) => {
  ...
  try {
    ...
    const response = await api.patch(`/admin-cta/update-active-specialist`, card);
...
const saveNewCard = async () => {
  loading.value = true;
  try {
    const response = await api.post(`/admin-cta/create-active-specialist`, newCard.value);
    if (response.data.success) {
...
const deleteCard = async (card) => {
  loading.value = true;
  try {
    const response = await api.delete(`/admin-cta/delete-active-specialist/${card.id}`);
    if (response.data.success) {
      const index = cards.value.findIndex(c => c.id === card.id);
      if (index !== -1) {
...
.specialist-card:hover .specialist-card-footer {
  opacity: 1;
  transition: all 0.3s ease;
}
